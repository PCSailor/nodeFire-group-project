...
router.post("/", function(req, res){
  ...
  // token email is identifier
  // need user & secret level
  var userEmail = req.decodedToken.email;
  // Check the user's level of permision based on their email
  User.findOne({ email: userEmail }, function (err, user) {
    if (err) {
      ...
      console.log(user);
      if(user == null) { // db has no object for logged in person
        // If the user is not in the database, return a forbidden error status
        console.log('No user found with that email. Have you added this person to the database? Email: ', req.decodedToken.email);
        res.sendStatus(403);
      } else {
        if(req.body.secrecyLevel > user.clearanceLevel) {
          console.log('req.body.secrecyLevel is : ' , req.body.secrecyLevel);
          console.log('user.clearanceLevel is : ' , user.clearanceLevel);
          // If the user has to low a secrecy level, return a forbidden error status
          console.log('no soup for you ', req.decodedToken.email);
          res.sendStatus(403);
        } else {
  ...
  // NOTE: B4 user secret level changes
  // router.post("/", function(req, res){
  //   console.log(req.body);
  //   var addedSecret = new Secret(req.body);
  //   console.log(addedSecret);
